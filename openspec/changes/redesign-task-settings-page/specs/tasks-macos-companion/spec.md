...
The macOS companion SHALL let the user record or clear one global keyboard shortcut from the Tasks Features card and SHALL use a configured shortcut to present the authoritative Tasks new-task editor from any active macOS application.
...
- **AND** entering the same shortcut already configured still commits the value and leaves recording mode after native confirmation

#### Scenario: Clear the shortcut
- **WHEN** the Global Quick Entry setting contains a shortcut and the user activates its interior clear control
- **THEN** the native companion removes the persisted global registration and the empty setting disables Global Quick Entry

#### Scenario: Present the shortcut inside Features
- **WHEN** Tasks runs in the native macOS companion
- **THEN** Global Quick Entry appears after the other applicable options inside the Features card with its existing explanation on a second line and a compact right-aligned shortcut control

#### Scenario: Withhold the setting outside native macOS
- **WHEN** Tasks runs in an ordinary browser, a PWA, or the iOS companion
- **THEN** the Global Quick Entry feature row is absent
