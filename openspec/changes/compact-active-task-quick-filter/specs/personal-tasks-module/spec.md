...
- **THEN** its name appears directly beneath the current list title, the icon-sized Quick Filters trigger uses the same active visual treatment as Select Tasks, and that trigger continues to allow the user to select another filter or All Tasks
...
- **THEN** the current list immediately returns to its ordinary unfiltered task membership, the active filter subtitle disappears, and the control returns to its inactive icon presentation
