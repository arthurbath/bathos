...
- [x] 4.1 Prove a clean install and valid dependency graph, then run the fresh audit, complete repository gate, applicable Tasks integration matrix, timing checks under the temporary performance waiver, strict OpenSpec validation, and Git diff checks.
  - Evidence: a fresh temporary-directory `npm ci` and full `npm ls --all` passed. The fresh audit reports 0 critical, 2 high, 3 moderate, and 1 low finding, all classified in the dated evaluation. The repository gate passed 170 test files and 1,424 tests with 8 files/15 opt-in tests skipped, lint with the one pre-existing Fast Refresh warning, Tasks typecheck, both Vite builds, 51 strict OpenSpec validations, and diff checks. Safari rendered the sign-in boundary and Terms dialog and correctly protected `/tasks/today`. Offline persistence, multi-client convergence, preservation/recovery, and the default ten-minute sustained test passed; the sustained run completed 300 cycles, 1,050 history records, 150 offline conflicts, 150 stale MCP conflicts, 300 capture retries, 300 transition retries, and 12 restarts. Cleanup readback found zero synthetic users, settings, tasks, history rows, disposable slots, publication, harness containers, and harness volumes. Advisory performance measured Upcoming-view p95 at 193.15 ms against the 100 ms ceiling and 1,000-row render at 3,666.76 ms against the 2,000 ms ceiling; both are permitted by the temporary waiver and retained for final baseline comparison.
