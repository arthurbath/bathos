...
The app declares only `os.bath.garden` in `WKAppBoundDomains` and configures the persistent Tasks web view to limit top-level navigation to that domain. This is required for WebKit to expose service workers to a native wrapper, allowing the existing Tasks offline shell to be staged and served from the same persistent browsing partition. External destinations never enter that restricted web view; the navigation delegate continues handing them to the operating system. A cold app-bound WebKit process can reject its first disconnected service-worker navigation even though the same cached route succeeds immediately afterward. The host therefore performs one bounded automatic recovery navigation before presenting its native unavailable treatment. A second failed navigation or terminated web-content process still resets the native presentation state so an unrecoverable load never leaves a blank surface or infinite spinner.

The cached shell stores the complete discovered Tasks asset graph under a dedicated offline URL namespace. Generated Vite preload tables can still request the same versioned files from their original root `/assets/` URLs, and worker code can resolve an `assets/` child beneath the offline namespace. For a root versioned asset, the service worker first checks whether that exact content-hashed resource exists in the active complete Tasks shell and returns it immediately when present; a cache miss continues to the network. This avoids waiting indefinitely for WebKit to reject a disconnected preload while keeping uncached shared assets and non-Tasks data APIs on their ordinary network paths. The worker also normalizes the one generated nested worker-asset form.
...
It never contains notes, primary links, checklist text, reminder credentials, mail-source metadata, Supabase tokens, PowerSync credentials, or secrets. The browser Push cleanup path treats a service-worker registration without `PushManager` support as having no browser subscription, which lets app-bound WebKit complete its owner-bound task-data and native-cache clearing without weakening endpoint revocation on browsers that do support Web Push. The native app atomically replaces the complete cache in the App Group and clears it on explicit web sign-out or owner change.
