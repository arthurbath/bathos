...
- Preserve manual task ordering inside each visible Upcoming date section.
- Allow a task to move between Upcoming sections by assigning the destination section's canonical future Start while retaining its Deadline.
- Keep Start authoritative over Deadline when deriving the one Upcoming section in which a task appears.
...
- `personal-tasks-module`: Add a durable automatic-list-sorting preference, deterministic Anytime and Someday ordering, constrained peer reordering, cross-Area drag projection, Upcoming date-section reordering, close-time reconciliation, and Actionability menu parity.
