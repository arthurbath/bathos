...
The BathOS root application SHALL resolve its development and production build
commands through one supported security-patched Vite release that is compatible
with the repository's Node runtime and installed Vite peer packages.
...
Ordinary BathOS development SHALL listen only on the local machine, while
network-accessible device testing SHALL require a separately named explicit
command.
...
The supported toolchain SHALL preserve BathOS development diagnostics and
generated integration behavior without changing production product semantics.
