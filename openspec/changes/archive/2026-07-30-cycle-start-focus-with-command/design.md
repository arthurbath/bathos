...
- Make every Today-horizon-to-tomorrow and month-to-month transition deterministic.
...
2. The focus sequence is derived from the focused element. Any focused Today horizon advances directly to the earliest legal calendar date, tomorrow, without cycling through other horizons. A focused calendar day advances by one calendar day. If focus is elsewhere in the picker, the command restores focus to the task's current Start choice rather than guessing a forward step.
