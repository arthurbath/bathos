...
Decorations use muted foreground styling by default, do not accept pointer events, are removed from the accessibility tree, and never enter the Tab order. Tasks may apply an existing semantic color when the decoration communicates a state that already uses that color elsewhere: Today horizons use their horizon colors, Waiting and Rechecking use the established purple, and a Deadline due today or earlier uses destructive red. Existing labels, placeholders, and `aria-label` values remain the accessible names. Right-side affordances and activation buttons retain their independent behavior.
...
### Completion boxes remain neutral

Task and checklist completion boxes use the same neutral gray in unchecked and checked states. Their hover state does not introduce green. Completion remains legible through the icon changing from the open square to the checked square, while semantic green remains available for higher-level success feedback rather than ordinary checkbox chrome. Selection-mode circles retain their existing information-blue treatment because they communicate selection rather than completion.
