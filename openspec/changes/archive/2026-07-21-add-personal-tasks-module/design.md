...
App-level shortcuts are inactive while focus is in an input, textarea, select, content-editable surface, or unrelated modal, except for the editor-specific save and cancel commands. Composition events are never interpreted as commands. The web client does not claim browser-critical shortcuts such as Command+1 through Command+9, Command+T, Command+L, Command+R, or Command+W. Tab and Shift+Tab retain native traversal, modified clicks retain native link behavior, every command has a visible control fallback, and supported controls expose `aria-keyshortcuts`. After completion, cancellation, recoverable deletion, or another action removes the focused row, focus moves to the row now occupying the same visual position, then the prior row, then capture or the view heading. The completion control and terminal row-menu actions share this restoration path rather than relying on focus behavior from an element that has just unmounted. Reordering retains focus on the moved task. Bulk-selection extensions may add commands later without changing this single-focus contract.

Structural and temporal movement surfaces capture the invoking row's visual position before applying their mutation. After a successful move closes the surface, focus returns to the task title when the task remains in the current view. If an optimistic planning update unmounts the row before the mutation promise settles, the surviving continuation restores focus to the same-position row, then the prior row, then capture or the view heading. Failed moves retain the command surface for correction or retry and do not advance row focus.
...
The Tasks trust suite also owns a dedicated TypeScript project that includes the complete module and task-specific MCP tools. BathOS's Vite build transpiles TypeScript without enforcing semantic type correctness, so this gate must pass independently and remain part of task-slice validation. It is intentionally scoped to Tasks and its transitive shared dependencies rather than making unrelated legacy module errors a prerequisite for Tasks development.

The connected template, recurrence, and reminder hooks have direct contract tests at the boundary between the reactive local projection and their server-authoritative services. The tests fix the clock across an owner-local date boundary, prove automatic and explicit recurrence evaluation, template anchor defaults, reminder claim and acknowledgement behavior, optimistic replacement, and refusal to mutate in local-only mode. Full-schema shared fixtures make additions to these server-backed records a compile-time test maintenance requirement rather than allowing stale partial mocks to conceal contract drift.
...
Structured capture now remains executable after it reaches the web interface. Active planning rows, Logbook rows, and project task rows share one origin indicator derived from `source_kind`. A supported `http:` or `https:` webpage or reading-item reference is a named real link that opens in a separate browser tab. A `message:` Mail reference or `file:` originating-Mac reference remains a real platform deep link without forcing a browser-tab target. Typed provenance with no usable reference, a malformed URL, or a source-incompatible protocol remains visible as a noninteractive named indicator. The interface never parses task titles or notes for a source, and the allowlisted protocol boundary prevents stored source metadata from becoming an executable arbitrary URL.
...
The first hierarchy workflow exposes `/tasks/projects`, `/tasks/areas/:areaId`, and `/tasks/projects/:projectId`. Areas, projects, and headings support creation, renaming, movement, and independent fractional ordering. An area opens from the hierarchy through a real link and presents only its present open loose to-dos and active projects. Loose work links to its currently derived planning view and restores stable task focus after ordinary navigation, while modified clicks retain browser behavior. Project details support ordered to-dos under optional headings plus independently editable, completable, reopenable, ordered, and recoverably deletable checklist items. Checklist deletion uses the same hierarchy-operation contract as other recoverable work, removes the item optimistically, rolls it back when the local write fails, and exposes the deleted root for restoration in Trash without changing the parent to-do lifecycle. Planning views show the current area, project, and heading context, and the ordinary to-do editor can move work between no container, an area, or a project heading without changing its planning placement or planning order. Repository reads normalize SQLite boolean projections before checklist validation so an unrelated edit or reorder remains valid after persistence. A desktop and mobile browser exercise with disposable synthetic data proved heading and task creation, renaming, movement, ordering, checklist completion and reordering, planning-context presentation, editor-based container movement, reload persistence, and mobile project navigation. A later rendered keyboard pass found that the browser did not reliably perform implicit form submission for inline hierarchy capture, so area, project, heading, project-to-do, and checklist-item fields now share one explicit Enter submission boundary while preserving IME composition.

A later requirement-to-rendered-behavior audit found that project planning state was complete in storage and MCP but absent from the web planning lists. The repaired projection now derives owner-safe project membership for Today, This Evening, Upcoming, Anytime, Someday, and Logbook using the same lifecycle, disposition, planning-date, and stable-order rules as to-dos. Each applicable view presents projects in a separately labeled section with real project-detail links, area and date context, project-specific temporal actions, and planning-order controls. Logbook can reopen projects through the hierarchy operation contract. Project detail now edits destination, Today section, start date, and deadline with the shared calendar control and date-range validation. To-do row focus, completion shortcuts, and bulk planning remain to-do-only instead of applying unsafe row operations to project hierarchies.
...
The implemented authoring model captures one current open to-do or project hierarchy as an immutable template revision. Capture stores relative start-date and deadline offsets from an explicit reference date, preserves actionability, ordering, headings, and checklist content, and deliberately resets generated checklist completion. Source revision and hierarchy serialization are resolved inside one private `STABLE` source-capture function invocation, so PostgreSQL applies one calling-statement snapshot across the root and descendant reads without blocking ordinary task edits. A template can be revised only through another explicit capture of the same kind. Authenticated clients can read synchronized definitions, revisions, and instantiation receipts, but they cannot write those tables directly. Capture, archive, and instantiation use guarded server functions so immutable history and generated provenance cannot be spoofed by a local queued write.
...
Calendar evaluation uses the definition's durable `evaluated_through_date` as a cursor. The server derives the first unevaluated recurrence step and the last step due through the requested date, then instantiates only the policy-selected range. `all` remains bounded by its explicit catch-up limit, while `latest` and `skip` select their candidate directly. A fresh request identifier therefore does not cause an old daily or weekly definition to rescan every interval from its original start date.

Editing a definition affects future, ungenerated occurrences only. Existing occurrences retain the revision and values from which they were created. Pausing or archiving a definition stops future generation without deleting existing work. Generation failure remains retryable and visible through content-free diagnostics. The web tracks evaluation failure independently from definition persistence: an accepted save or resume remains accepted when its immediate catch-up fails, the repeat row displays the degraded catch-up state, and the existing catch-up control becomes an explicit retry. Automatic evaluation attempts at most once per definition and planning date after a failure so a persistent service error cannot create a render-driven retry loop. Recurrence projection loading and failure remain distinct from an empty definition list, and authoring controls stay unavailable until that projection can be trusted.
...
The web exposes one global Undo action for the latest eligible to-do mutation in its synchronized local projection. Its reactive query joins accepted history to the current task revision, excludes baseline, creation, and prior undo events, and therefore withholds the action while a newer local mutation is unacknowledged or no authoritative inverse exists. The visible header action and Command+Z or Control+Z outside editable controls call the same repository inverse; editable controls and command surfaces retain native undo behavior. Once an accepted event has synchronized, the inverse remains available offline and queues through the ordinary task mutation path. Local-only mode does not invent accepted history merely to enable the control.
...
Permanent deletion is a connected, server-authoritative web operation for deleted to-do and project roots. The client first requests a content-bounded preview containing every hierarchy identifier in the deletion root, every related history, Mail-source, reminder, occurrence, and delivery identifier that will be erased, the content-free integrity receipts that will remain, and a digest of that exact scope. Execution requires a new request UUID, the unchanged digest, the exact non-null confirmation phrase, an active synchronization connection, and an empty local upload queue. The server rejects every other confirmation value before scope lookup, then locks and recomputes the scope in the execution transaction and rejects a stale preview before deleting anything. An exact execution retry returns the original content-free receipt.
...
Restore validates checksums and schema compatibility before writing, supports a dry run, assigns all imported data to the authenticated owner, and never trusts exported owner identifiers. Merge restore is idempotent by stable identifier and reports conflicts without overwriting newer records. Replace restore is limited to the complete current schema. Preparation validates the incoming envelope and returns a checksum-verified, transaction-consistent current server export plus a digest that excludes only the export timestamp without taking the global task write lock. The interface requires that backup to be downloaded and uses a separate exact confirmation. Execution takes the global write lock, rejects a changed server digest, and deletes and restores the owner task graph in one transaction. A failed restore rolls the deletion back. Browser delivery targets and Web Push credentials remain registered because they are deliberately excluded from portable task backups, while old task-specific delivery attempts and claims are removed. A private content-free receipt makes an exact retry after an ambiguous response idempotent.
...
The service records scheduled, attempted, provider-accepted, failed, and acknowledged states separately. Provider acceptance is not reported as proof that the user saw the notification. Permission denial, missing platform support, expired targets, and persistent delivery failure are visible as degraded reminder capability without blocking task operation. The open web client also reports a failed due-delivery claim through fixed content-free copy and a bounded retry action; it does not expose the provider error, remove existing claimed items, or imply that the scheduled reminder itself was lost. A due-claim request has a ten-second client deadline, aborts its underlying Supabase request when supported, and always releases the in-flight guard so Retry cannot remain disabled behind an indefinitely stalled network call. Visible and notification-opened acknowledgement failures use the same content-free boundary, release the notification-open retry guard, and leave the claimed reminder available for another attempt.

The completed reminder slice stores one active reminder per to-do or project, including its original local date, wall-clock time, IANA time zone, ambiguity choice, resolved UTC instant, and resolution kind. Each revision creates one immutable logical occurrence. Per-target delivery rows are unique by occurrence and registered target. To-do editing and open project detail both expose the same connected-only reminder contract; project detail can create, revise, or clear a project-root reminder, reports the owner planning time zone and daylight-saving adjustment, and remains visibly unavailable in local-only mode rather than inventing local delivery identity. Both editors derive one shared reminder-availability state and disable mutation while the current projection is loading or unavailable, preventing an unknown existing schedule from being treated as no schedule. The in-app client claims due delivery rows through a short server lease and an immutable request receipt, which prevents multiple tabs from creating another logical delivery while permitting recovery after an abandoned attempt. Portable export version 10 includes reminder intent and occurrence history but deliberately excludes delivery endpoints, subscription material, attempt diagnostics, and claim receipts.

Web Push registration is an explicit browser action. On ordinary Tasks startup, the client uses a passive service-worker-registration lookup to inspect an existing subscription and never installs the worker as a side effect of viewing the module. The Enable action performs feature detection, requests notification permission from that user gesture, registers one standards-based service-worker subscription, and reports unsupported, denied, unconfigured, expired, and failed capability states without blocking task work. Failed inspection, registration, or revocation keeps the underlying exception internal while the persistent capability panel and transient toast use fixed content-free copy, preserve in-app reminder availability, and expose a safe explicit retry. Provider endpoint and encryption material live in a separate RLS-enabled table with no authenticated table-read grant and do not enter the synchronized target projection. The synchronized target retains only a hashed endpoint identity, a user-facing device label, preview choice, and capability status.

One physical browser subscription belongs to at most one BathOS owner at a time. Registration serializes on its hashed endpoint identity, cancels outstanding delivery for any prior owner, removes the prior provider credential, marks the prior target revoked, and then assigns the subscription to the current owner. Tasks sign-out performs the owner-scoped server revocation while authentication is still available and unsubscribes the browser endpoint before clearing the local database. Platform sign-out also performs a passive user-bound push unsubscribe so leaving BathOS from another module cannot leave a prior account receiving task-title notifications on that installation.

A service-role-only lease operation creates or reuses one delivery row for each due occurrence and active Web Push target. The scheduled Edge Function sends the encrypted notification in bounded batches, reuses the delivery identifier as the provider topic on retry, records provider acceptance separately from user acknowledgement, preserves content-free operational logs, degrades transient failures, and revokes expired endpoints on provider HTTP 404 or 410. Immediately before the network sink, the dispatcher parses the claimed endpoint and permits only HTTPS endpoints on provider-controlled Apple, Google, Mozilla, or Microsoft push-service hosts. It rejects credentials, non-provider hosts, deceptive suffixes, and non-HTTPS URLs without making a request, then records a content-free terminal failure that revokes the target. This sink-side guard preserves the standards-based opaque subscription contract while preventing an authenticated caller from turning the service-role dispatcher into an arbitrary network client. Opening a notification deep-links to the relevant Tasks route and acknowledges the logical occurrence so a later in-app claim does not present it again. The public VAPID key is client-safe; the matching private key and Cron dispatch secret remain managed deployment secrets. A missing key, schedule, browser permission, or provider capability is visible as degraded or unconfigured operation rather than being mistaken for successful delivery.

Notification-click routing treats the payload destination as untrusted input. It accepts only the current BathOS origin and `/tasks` route family, falling back to Today for malformed, cross-origin, or non-Tasks destinations. The service worker reuses only an existing Tasks window; another BathOS module remains on its current route and a new Tasks window opens instead. A failed attempt to navigate a stale Tasks client also falls back to a fresh Tasks window so the reminder does not become a dead click.

Because the reminder worker contains backward-compatible push display and notification-click behavior, its install event requests immediate activation with `skipWaiting`. This makes a published reminder fix active without requiring every open BathOS tab to close. The worker deliberately does not call `clients.claim()`, so activating the notification worker does not eagerly take control of unrelated BathOS pages.

The production reminder package keeps credentials out of the repository, requires the server and web build to share one verified public VAPID key, proves that the private key matches it, and requires an independent dispatch secret of at least 32 bytes. Cron resolves that secret from Supabase Vault at invocation time, so the decrypted value is absent from migrations and the stored job command. A fixed-name, once-per-minute job calls only the approved BathOS function endpoint. Database verification rejects absent or duplicate Vault values, inactive or duplicate jobs, schedule or endpoint drift, and any job command containing the decrypted secret. The dispatcher accepts current hosted and local Supabase server-key shapes, compares the custom dispatch secret without length-sensitive early exit, and returns an HTTP failure when a provider outcome cannot be recorded. Production provisioning and real-device acceptance were approval-gated external actions and are now complete. Future credential rotation, provider changes, or topology changes require a fresh approval.

The approved production exercise registered Safari through an explicit permission gesture, received one synthetic notification in Safari, opened the notification deep link, recorded one provider attempt separately from acknowledgement, proved the expired-target revocation receipt on an isolated synthetic endpoint, and removed every task-specific fixture while retaining the real browser target and credential. The first notification opening exposed a synchronization boundary mismatch: PowerSync represents PostgreSQL `time` values with fractional seconds, while the browser validator accepted only minute or whole-second precision. The client now accepts bounded fractional-second precision as the same wall-clock intent. Regression coverage, a live redeployment, successful route rendering, and the acknowledged production receipt close that finding.
...
The 2026 Jul 20 deployment refresh recommended PowerSync Cloud Free for the bounded parallel-use trial. The expected one-owner workload fits its published capacity, Things remains the fallback during deactivation or another outage, and managed service avoids assuming an untested operations burden. The owner later approved the processor, production replication boundary, and synthetic production-path validation. An authoritative BathOS task system must later move to managed service without inactivity deactivation or a monitored and recoverable self-hosted topology.

A preactivation read-only production audit later on 2026 Jul 20 confirmed that the Bath Supabase organization was already on Pro and BathOS was active on Postgres 17, resolving the plan-level external-replication prerequisite without a plan change. No Tasks replication role, publication, PowerSync endpoint, reminder dispatcher, reminder Vault entry, Cron schema, `pg_cron`, or `pg_net` existed at that point, so activation followed the fresh-install path. The owner later approved and completed those production changes.
...
The dedicated replication login receives explicit `SELECT` on the exact synchronized Tasks relation set and no role memberships. Deployment verification evaluates usable relation and column privileges together with schema usage, rejects direct grants outside that set, rejects public `SECURITY DEFINER` application functions, and fails on any new schema or relation. Hosted Supabase nevertheless grants `PUBLIC` access to the `supabase_admin`-owned `net` schema, its two operational queue tables, and its request functions. The project `postgres` role cannot revoke the managed owner-issued grant. The verifier therefore permits only that exact owner-controlled pg_net surface as a documented infrastructure exception while keeping it outside the publication and Sync Streams. Eliminating the exception requires Supabase support or replacement of the reminder scheduler; the repository does not claim an ineffective SQL revocation.

Production hardening on 2026 Jul 21 applied the six pending Tasks migrations, advanced the reminder dispatcher to version 2, and normalized the replication role and existing publication. The first normalizer run exposed that the provisioner always selected the fresh-install publication path. PostgreSQL rejected the duplicate publication before changing it. The provisioner now validates the remote publication-exists result and selects the create or update path explicitly. The corrected verifier reported `ready` with exactly 22 synchronized tables. The reminder verifier reported one active minute schedule and three recent successful runs, while the synthetic production topology gate again passed convergence, isolation, restart, and cleanup. Supabase advisors report no remaining Tasks RLS initialization-plan finding.
...
The implemented header status is an accessible dialog trigger backed by PowerSync's reactive transfer status, the durable upload-queue count, and the local content-free conflict-receipt table. Each tab installs its PowerSync status listener and then reconciles the database's current status, preventing a tab that joins an already-running shared-worker session from remaining in its synthetic Connecting state after the worker's latest status event has passed. The compact label elevates upload or download failure rather than presenting a misleading connected state. Details report both transfer directions independently, the last successful synchronization, pending changes, and the ten most recent conflict receipts without rendering provider error text or task content. In local-only mode, the same surface explicitly states that cross-device and MCP changes do not converge.

Production acceptance on 2026 Jul 21 deployed the current-status reconciliation and bounded reminder claim under deployment `4c8c647e-9d5e-42c4-9d64-be07fc73ee5c`. Reloading one authenticated Safari Tasks tab while a second Tasks tab remained open caused one shared reconnect that both tabs reported truthfully, after which both returned to Synced with no pending changes. The reloaded tab's stalled reminder claim reached the content-free failure state with Retry enabled, and the failure cleared after a later successful claim without changing task data.
...
Production Raycast acceptance on 2026 Jul 20 completed dynamic OAuth registration in Safari, stored the rotating refresh grant in the login Keychain, created one Inbox task through `create_task`, projected it into the synchronized web Inbox, and permanently removed the synthetic hierarchy and related history through the web cleanup path. The first attempt exposed stale production MCP deployment plus invalid generated `npm:@/` imports for shared Tasks domain modules. MCP sources now use relative domain imports so the generator inlines them into the Edge Function. A deployment contract test requires `create_task`, the inlined domain modules, and the absence of invalid BathOS npm aliases.

The hierarchy-creation slice adds four explicit tools rather than reversing that generic-CRUD boundary: `create_task_area`, `create_task_project`, `create_task_heading`, and `create_task_checklist_item`. Every tool requires a caller-generated UUID idempotency key, fixes provenance to MCP automation, generates the record identifier and append order server-side, and returns the immutable hierarchy creation receipt plus the current owner-safe record. Project input exposes only supported area, content, date-only planning, and planning-view fields. Headings require a present open owned project, and checklist items require a present open owned to-do. Exact retries resolve `tasks_hierarchy_history_events`, so a later edit does not defeat deduplication; changed data or a different hierarchy record type for the same key is rejected.

The hierarchy content-update slice remains explicit through `update_task_area`, `update_task_project`, `update_task_heading`, and `update_task_checklist_item`. Area and heading updates rename only; project updates allow title and notes; checklist updates allow title and coupled completion state. Every call requires the record's current positive revision and a logical mutation UUID. A stale request returns current owner-safe state with a content-free conflict receipt, an already-current request returns a no-op without refreshing a checklist completion timestamp, and an accepted exact retry resolves immutable hierarchy history even if the current record or parent later changes. Accepted writes fix MCP automation provenance and rely on the authoritative hierarchy revision and history triggers. Movement, planning, lifecycle, deletion, and recovery remain separate operations rather than hidden fields on these content tools.

The hierarchy lifecycle and recovery slice adds one closed `transition_task_hierarchy` operation for areas, projects, headings, and checklist items while leaving to-dos on `transition_task`. Project completion and cancellation default to rejecting open descendants and require an explicit cascade to transition them; reopening never guesses that descendants should reopen. Recoverable deletion and restoration always use the structural cascade, and permanent deletion is absent. The MCP caller supplies only the current root revision. One narrowly granted authenticated Postgres function derives the complete owner-scoped candidate revision set server-side, substitutes the caller's expected root revision, and submits the existing atomic hierarchy-operation receipt. This avoids Data API pagination limits, makes newly changed descendants an authoritative conflict rather than a partial mutation, fixes provenance to MCP automation, and returns immutable exact retries without recreating work.

Project movement and scheduling remain distinct through `move_task_project` and `schedule_task_project`. Movement accepts an optional complete area placement plus a supported project planning destination, derives Today from the owner's planning time zone, and generates structural or planning tail keys instead of accepting raw order values. Scheduling changes only date-only start and deadline values, activates a scheduled Someday project into Anytime, and preserves the established project planning constraints. Both operations require an open present project and its current positive revision, fix provenance to MCP automation, resolve accepted retries from full immutable hierarchy-history snapshots, reject cross-surface mutation-key reuse, and return content-free no-op or conflict receipts without forging history.

Manual ordering is exposed separately through direction-based `reorder_task` and `reorder_task_hierarchy` tools. Callers supply `up` or `down`, a stable record identifier, current revision, and logical mutation UUID, never a raw fractional key or destination index. To-dos choose either planning order or hierarchy order; projects choose structural or planning order; areas, headings, and checklist items expose only their structural peer order. Planning requests also carry the explicit view and planning date so Today section membership and owner-local date rollover cannot change the meaning of an accepted retry. The server reads each exact owner-scoped peer collection in stable ordered pages, derives the adjacent move, and updates only the selected ordering dimension. Collection boundaries are content-free no-ops, stale revisions are conflicts, and accepted retries validate the immutable before-and-after history snapshot before returning the original receipt plus current owner-safe state.
...
All accepted MCP mutations fix the actor to `automation`. Update, movement, schedule, and lifecycle operations fix the mutation channel to `mcp`; hierarchy creation also fixes both entry and mutation provenance to `mcp`, while to-do creation records either the declared closed integration channel or the default `mcp` channel. Results include actor, channel, affected stable identifiers, base and resulting revisions, transition, occurrence time, outcome, applicable code, and the current owner-safe record. Creation results use the same complete receipt shape. Deterministic no-op, rejected, and stale outcomes are content-free and do not forge append-only history.
...
- Which native Apple surface, if any, will normal parallel use prove valuable enough to justify the first companion build?
