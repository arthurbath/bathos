...
- **THEN** the editor presents an ordinary proportional bullet marker in place of the source asterisk
- **AND** wrapped continuations align after that visible bullet and normal space rather than retaining the wider two-fixed-character source indent
...
- **THEN** the editor inserts a new line beginning with `* ` and wraps each active source bullet with a two-fixed-width-character hanging indent
