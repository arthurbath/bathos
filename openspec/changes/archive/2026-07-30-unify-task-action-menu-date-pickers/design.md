...
The existing Start advancement event remains panel-local. Control+E first advances an already-open Start picker and only dispatches a row-open command when no Start picker is active. The shared date-picker panel exposes an equivalent non-committing date-advance event. Control+D sends that event to an active task Deadline panel before it considers opening another Deadline surface, so open-editor, anchored-row, and centered selection-mode entry points all advance one day without toggling closed.
...
- [Repeated Deadline advancement could accidentally persist a date] -> Advance only the calendar's controlled month and exact-focus request; retain Enter, Space, or pointer selection as the sole commit paths.
