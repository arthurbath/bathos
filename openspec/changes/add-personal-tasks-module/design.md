...
Mail capture persistence and lifecycle services are ready, but Inbox Manager dual writing remains gated on explicit parallel-use approval. The append-only retirement audit collection also needs to enter a later portable export version before replacement readiness. Existing Mail rules and Things delivery remain unchanged.
