...
Mail is deliberately not activated in this slice. The observed Inbox Manager contract needs a durable message identifier, Mail account identity, deep link, and integration-specific source-retirement state. The current production task row can preserve only a source kind, URL, title, and external identifier, so encoding the missing fields into opaque strings would lose domain meaning. Mail capture and Inbox Manager dual writing remain gated until a structured-source expansion represents that lifecycle honestly and the BathOS Inbox is approved for parallel use. Existing Mail rules and Things delivery remain unchanged.
