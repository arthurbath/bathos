...
- Keep the existing `/tasks/projects` destination focused on Projects, with Project iconography and no Area-administration controls.
- Move Area creation and management to Config, including rename, recoverable delete, and manual prominence ordering.
- Keep Today grouped only by Inbox, Now, Next, and Later, with Area and Project names presented as secondary task metadata.
- Divide Anytime and Someday into an unlabelled No Area region followed by one task bucket per Area that has visible tasks.
- Order Anytime and Someday Area buckets by the manual Area order maintained in Config.
...
- Projects and Config navigation labels, page copy, breadcrumbs, iconography, human documentation, and focused tests.
