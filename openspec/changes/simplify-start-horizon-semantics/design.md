...
The database migration will clear horizons from every current future-start to-do and project, replace planning constraints and normalization triggers, and change reached-date activation to assign Next. Export, restore, template, recurrence, MCP, and undo or redo normalization must emit only canonical forms. Existing reminders remain anchored to a future Start and are not canceled merely because its obsolete future horizon is cleared.
