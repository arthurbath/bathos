...
Run `node scripts/provision-tasks-production.mjs verify-sync-database` for a read-only production check of the effective role, exact 20-table publication, RLS and replica identity, and the documented Supabase-managed `pg_net` exception. Unlike `sync-database`, this verification command never normalizes the role or changes the publication.
...
The historical structure-simplification gate remains documented in its archived release evaluation. Current production acceptance uses the project-free topology gate and schema-13 export contract.
