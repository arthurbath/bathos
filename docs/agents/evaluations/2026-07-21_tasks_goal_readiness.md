...
The first actual iPhone Home Screen pass then failed immediately after Airplane Mode with Safari's standard no-network page. Apple documents that standalone Home Screen web apps keep cookies and storage separate from Safari, so the successful Safari cache inspection did not prove readiness in the installed app's partition. The acceptance instructions also asked only for one online Home Screen launch and exposed no way to know whether that app's worker had activated and completed staging before iOS suspended it. The corrective implementation retains the permanent same-origin `/tasks/manifest.json`, waits up to 30 seconds for the registered worker and atomic shell cache, and reports the current partition as `Offline Launch: Preparing`, `Ready`, `Failed`, or `Unavailable` inside Synchronization Details. Focused partition and delayed-staging tests, the full 705-test suite, lint, production build, and strict OpenSpec validation pass locally. Publication and a repeated device pass remain pending.
