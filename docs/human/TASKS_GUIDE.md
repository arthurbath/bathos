...
Structured webpage, file, reading-item, and Mail sources remain distinct from the task title and notes. Primary Link is the editable shortcut shown on the task row: `message:` values use the Mail icon, `jira:` and recognized Jira web URLs use the Zap icon, `obsidian:` values use the File Text icon, generic HTTP(S) values use the Link 2 icon, and other nonblank values open as HTTPS destinations. The same derived identity icon appears inside the Primary Link input, on the task row, and on supported native widget surfaces, while the adjacent drawer action always uses the External Link icon to communicate that it opens the destination. Mail capture initializes Primary Link from its audited message deep link without replacing source provenance. Expanded notes are always directly editable and style supported Markdown as you type without hiding its source. The supported subset is headings, asterisk emphasis, double-asterisk strong text, asterisk bullets, Markdown links, and inline code. Heading, emphasis, strong, bullet, and link indicators use a fixed-width font. Inline code uses a fixed-width font and a light background across the complete delimited string. Safe HTTP(S), `message:`, `jira:`, `obsidian:`, and other supported application links remain actionable. Press Enter in an asterisk bullet to begin the next bullet.
