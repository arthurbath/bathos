-- Fractional-indexing keys are ordinal byte strings. Match the client and the
-- Upcoming RPC by forcing PostgreSQL's C collation throughout activation.
  ...
  LOOP
    ...
    FOR _recurrence IN
      ...
      ORDER BY
        ...
        definition.upcoming_order_key COLLATE "C",
        definition.id
    LOOP
    ...
    FOR _task IN
      ...
      ORDER BY
        ...
        COALESCE(task.upcoming_order_key, task.order_key) COLLATE "C",
        COALESCE(
          (
            SELECT 'recurrence:' || occurrence.recurrence_id::text
            FROM public.tasks_recurrence_occurrences AS occurrence
            WHERE occurrence.owner_id = task.owner_id
              AND occurrence.root_type = 'todo'
              AND occurrence.root_id = task.id
