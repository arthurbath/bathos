...
describe("toast duration", () => {
  it("uses the two-second base when a toast has no readable text", () => {
    expect(getToastDurationMs()).toBe(2_000);
  });

  it("shows one short text block for two seconds", () => {
    expect(getToastDurationMs("Saved")).toBe(2_000);
    expect(getToastDurationMs(undefined, "The task now appears in Upcoming.")).toBe(2_000);
  });

  it("counts a title and description as separate visible lines", () => {
    expect(getToastDurationMs("Operation Failed", "Please try again.")).toBe(3_500);
  });
  ...
  it("combines wrapped title and description estimates", () => {
    ...
    expect(getToastDurationMs(wrappedTitle, "One description line")).toBe(5_000);
  });
  ...
  it("reads structured React content and line breaks", () => {
    ...
    expect(getToastDurationMs(content)).toBe(3_500);
  });

  it("reads deferred toast content", () => {
    expect(getToastDurationMs(() => "Deferred title", () => "Deferred description")).toBe(3_500);
  });
});
